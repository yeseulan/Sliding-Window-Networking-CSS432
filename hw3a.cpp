 ...
 * CSS 432
 * Assignment 3
 * The main program that runs stop-and-wait algorithm, sliding window algorithm, sliding window with drop rate
 *  and prints out their performance
 */
...
int main( int argc, char *argv[] ) {
    ...
    cerr << "   4: sliding windows with drop(from 0 to 10 rate)" << endl;
    cerr << "--> ";
    cin >> testNumber;
    ...
    if ( myPart == SERVER ) {
        switch( testNumber ) {
            ...
            case 4:
                for ( int windowSize = 1; windowSize <= 30; windowSize=windowSize+29) {
                    for (int drop = 0; drop <= 10; drop++) {
                        serverEarlyRetrans(sock, MAX, message, windowSize, drop);
                    }
                }
                break;
            default:
